...
* `company_type`: text field, type of company. There are 66 unique types, I've pulled them out of the entities file into [distinct_company_types.txt](distinct_company_types.txt). Seems to be somewhat arbitrary
...
* `status`: text field, 84 unique values. Seems to be arbitrary, see [distinct_company_status.txt](distinct_company_status.txt)
...
* `status`: text field, 10 unique fields, over 14,000 null values. Common: `ACTIVE,SUSPENDED`. See [intermediaries_distinct_status.txt](intermediaries_distinct_status.txt)
...
* `countries` and `country_codes`: see above. A rough sampling shows high correlation between the country identified in the `name` and/or `address` and the `country_codes`. There *are* 7 records that have 2 country_codes (separated by a semicolon) and indeed have two addresses is the `address` field. That's pretty ugly, see [address_multiple_country_codes.txt](address_multiple_country_codes.txt)

### Relationships (relationships.csv)
Relationships for all the nodes (~3.3M records)

* `node_id_start`: this is 'from' node id, never null
* `node_id_end`: this is the 'to' node id, never null
* `rel_type`: text field, no nulls, 14 unique values. See [relationship_types.txt](relationship_types.txt). TODO: understand the meaning of all these types
* `link`: text field, has nulls. Seems to be a a re-wording of `rel_type`. Some values are length (281 chars)
